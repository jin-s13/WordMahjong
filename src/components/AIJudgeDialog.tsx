...
interface AIJudgeDialogProps {
  isOpen: boolean;
  sentence: string;
  onClose: () => void;
  onConfirm: (result: ValidationResult) => void;
  validateFunction: (sentence: string, userExplanation?: string) => Promise<ValidationResult>;
  addPunctuation: (sentence: string) => string;
}

const AIJudgeDialog: React.FC<AIJudgeDialogProps> = ({
  isOpen,
  sentence,
  onClose,
  onConfirm,
  ...
  const validateSentence = async (userExplanation?: string) => {
    setStatus('validating');
    setValidationResult(null);
    
    try {
      const result = await validateFunction(sentence, userExplanation);
      setValidationResult(result);
      setFormattedSentence(addPunctuation(sentence));
  ...
  const handleAppeal = async () => {
    ...
    setIsAppealing(true);
    await validateSentence(appealExplanation);
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-grow">
          ...
          <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
            <h3 className="text-sm font-medium text-gray-600 mb-2">您的句子：</h3>
            <p className="text-xl font-serif text-gray-900 leading-relaxed">
              "{sentence}"
            </p>
            {formattedSentence && (
              <div className="mt-3 pt-3 border-t border-gray-200">
                <p className="text-sm text-gray-500 mb-1">添加标点后：</p>
          ...
          {status === 'validating' && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">AI裁判正在分析句子语义...</p>
              <p className="text-sm text-gray-500 mt-2">正在检查语法正确性和语义完整性</p>
            </div>
          )}
          ...
          {validationResult && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <span className="text-gray-700 font-medium">语义评分：</span>
                <div className="flex items-center gap-3">
            ...
            {status === 'error' && (
              <>
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    console.log('申诉按钮被点击，当前showAppeal:', showAppeal);
                    setShowAppeal(prev => !prev);
                  }}
          ...
          {showAppeal && (
            <div className="mt-4 p-4 bg-amber-50 rounded-lg border border-amber-200">
              ...
              <textarea
                ...
                placeholder="请详细解释这句话的含义、使用场景或者表达的意思，帮助AI裁判理解..."
                rows={3}
